...
const trendChannelLens = Lens.fromPath();
const trendChannelKeyPointsLens = trendChannelLens(['keyPoints']);
const trendChannelFillBGLens = trendChannelLens(['properties', 'style', 'fillBackground']);
export const TrendChannelSettings = memo((props) => {
    ...
    const updateCoordinates = useCallback((keyPoints) => createHandler(trendChannelKeyPointsLens, keyPoints), [createHandler]);
    const updateFillBackground = useCallback((isFillBackground) => createHandler(trendChannelFillBGLens, Boolean(isFillBackground)), [createHandler]);
            ...
            React.createElement(LineStyleGroup, { value: value, onValueChange: onValueChange, palette: palette },
                React.createElement(DrawingSettingsItem, null,
                    React.createElement(DrawingSettingsCheckboxStyled, null,
                        React.createElement(Checkbox, { value: drawingProperties.style.fillBackground, onValueChange: updateFillBackground }),
                        React.createElement("span", null, styleSectionDict.backgroundCheckbox))))),
