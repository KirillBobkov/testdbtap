...
import { MonadTask, MonadTask1, MonadTask2, MonadTask2C, MonadTask3 } from 'fp-ts/MonadTask';
...
export interface FromEvent<M> {
    <K extends keyof WindowEventHandlersEventMap>(target: WindowEventHandlers, event: K): HKT<M, WindowEventHandlersEventMap[K]>;
    (target: EventTarget, event: string): HKT<M, Event>;
}
export interface FromEvent1<M extends URIS> {
    <K extends keyof WindowEventHandlersEventMap>(target: WindowEventHandlers, event: K): Kind<M, WindowEventHandlersEventMap[K]>;
    (target: EventTarget, event: string): Kind<M, Event>;
}
export interface FromEvent2<M extends URIS2> {
    <E, K extends keyof WindowEventHandlersEventMap>(target: WindowEventHandlers, event: K): Kind2<M, E, WindowEventHandlersEventMap[K]>;
    <E>(target: EventTarget, event: string): Kind2<M, E, Event>;
}
export interface FromEvent2C<M extends URIS2, E> {
    <K extends keyof WindowEventHandlersEventMap>(target: WindowEventHandlers, event: K): Kind2<M, E, WindowEventHandlersEventMap[K]>;
    (target: EventTarget, event: string): Kind2<M, E, Event>;
}
export interface FromEvent3<M extends URIS3> {
    <R, E, K extends keyof WindowEventHandlersEventMap>(target: WindowEventHandlers, event: K): Kind3<M, R, E, WindowEventHandlersEventMap[K]>;
