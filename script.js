...
async function connect(){
    ...
    console.log('Берем сервис ', serviceUUID);
    const service = await server.getPrimaryService(serviceUUID);

    console.log('Берем характеристику ', serialUUID);
...
function read(event) {
    ...
    console.log('Кодировка сообщения 1: ', decodedMessage);

    let value = event.target.value;
    console.log('Кодировка сообщения 2: ' + new TextDecoder().decode(value));
